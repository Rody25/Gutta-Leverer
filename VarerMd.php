...
<body>
    ...
    <div class="listebanner1">
        <p class="logo">Drikke:</p>
        <div class="cart">
    ...
    try {
        $stmt = $conn->prepare("SELECT ProductID, Name AS title, Price, Image FROM Products WHERE Category = 'Drikke' AND StoreID = 'M'");
        $stmt->execute();
    ...
    function addToCart(productID) {
        ...
        if (!item) return;
        cart.unshift(item); // Adds the new item to the top of the cart
        displayCart();
    }
    ...
    document.getElementById("root").innerHTML = products.map((item) => {
        return `
                <div class="box">
                    ...
                    <p>${item.title}</p>
                    <h2>${item.Price}kr</h2>
                    <button class="bm" onclick="addToCart('${item.ProductID}')">Add to cart</button>
                </div>`;
    }).join("");
    </script>
